...
### 置顶与防截屏
- **双版轮询维持置顶（6-28）**：UIAccess 版 3000ms，便携版 250ms 
- **`WM_WINDOWPOSCHANGED` 事件驱动**：只在被挤下去时才重新置顶
- **防截屏定时重设**：主窗口 + 悬浮窗定时器循环调用 `SetWindowDisplayAffinity`，防止被外部清除
...
### UI 优化（6-28）
- **退出黑屏移至高级工具**：不占用极域控制组空间
- **新增显隐悬浮窗按钮**：高级工具内一键切换悬浮窗显示/隐藏
- **杀机房助手提升到主界面**：极域控制组内直接操作
- **窗口始终左上角**：每次打开复位到 (0, 0)
...
### 文档
- **README 目录**：章节索引快速跳转
- **RELEASE.MD文件**：发行版说明，面向最终用户
- **DEV.MD文件**：开发者文档（编译部署、脚本、故障排查、附录）
