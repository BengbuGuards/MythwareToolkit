...
LRESULT CALLBACK WndProc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam) {
	switch (Message) {
		case WM_CREATE: {
			...
			BtSelWnd = CreateWindow(WC_BUTTON, "选择窗口透明", WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_PUSHBUTTON, 264, 200, 248, 22, hwnd, HMENU(1021), hi, NULL);
				...
				case 1007: {
					...
					static const std::pair<LPCSTR, std::vector<std::pair<LPCSTR, LPCSTR>>> paths[] = {
						...
						{"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Policies\\Explorer", {
							...
							{"Hidden", "强制显示隐藏文件"}, //另有CurrentVersion\Explorer\Advanced\Hidden、ShowSuperHidden和HideFileExt三个选项，因属于用户设置，可手动解禁，而不作处理。
					...
					static const std::pair<LPCSTR, std::vector<std::pair<LPCSTR, LPCSTR>>> deletePaths[] = {
						{"SOFTWARE\\Policies\\Google\\Chrome", {
							...
							{"DownloadRestrictions", "Chrome下载限制"},
							{"SaveAs", "Chrome另存为"},
							{"DeveloperToolsAvailability", "Chrome开发者工具"}}},
						{"SOFTWARE\\Policies\\Microsoft\\Edge", {
							{"AllowSurfGame", "Edge冲浪游戏"},
							{"WebWidgetAllowed", "Edge桌面栏"},
							{"DownloadRestrictions", "Edge下载限制"},
							{"SaveAs", "Edge另存为"},
							{"DeveloperToolsAvailability", "Edge开发者工具"}}},
						{"SOFTWARE\\Policies\\Mozilla\\Firefox", {
							{"DisableDownloads", "Firefox下载限制1"},
							{"BlockAboutDownloads", "Firefox下载限制2"},
							{"DeveloperToolsAvailability", "Firefox开发者工具"}
						}},
     ...
     while (ReadFile(hFile, buf, sizeof(buf), &read, NULL) && read > 0) {
				...
				case 1013: {
					...
					//注意：v11开始，机房管理助手安装文件夹中的重要文件，全部加FILE_ATTRIBUTE_SYSTEM和FILE_ATTRIBUTE_HIDDEN属性，其他文件用于混淆。
				...
				case 1021: {
					if (!IsWindows10OrGreater()) {
						SetWindowText(TxOut, "此功能需要 Windows 10 及以上版本支持");
						break;
					}
					if (selectingWindowForAffinity) {
						SetWindowText(TxOut, "正在等待窗口选择，请完成当前操作。");
						break;
					}
					...
					if (!hWindowSelectHook) {
						...
						hPrevCursor = NULL;
						PrtError("安装窗口选择钩子失败", 0);
						SetWindowText(TxOut, "无法启动窗口选择。");
						break;
					}
					selectingWindowForAffinity = true;
					SetWindowText(TxOut, "提示：左键选择窗口，右键取消。");
					break;
				}
			case 1014: {
					//找到工具条
		...
		case WM_SELECT_WINDOW_DONE: {
			...
			if (lParam == 1) {
				SetWindowText(TxOut, "已取消窗口选择。");
				break;
			}
			HWND target = HWND(wParam);
			if (!target || !IsWindow(target)) {
				SetWindowText(TxOut, "未找到有效的目标窗口。");
				break;
			}
			...
			if (!GetWindowText(target, title, sizeof(title)) || !title[0])
				lstrcpyn(title, "未命名窗口", sizeof(title));
			...
			if (it == gExcludedFromCapture.end()) {
				if (SetWindowDisplayAffinity(target, WDA_EXCLUDEFROMCAPTURE)) {
					...
					sprintf(status, "已将窗口[%s]设置为截图透明。", title);
					SetWindowText(TxOut, status);
					Println(status);
				} else {
					PrtError("设置截图透明失败。", 0);
					SetWindowText(TxOut, "设置截图透明失败。");
				}
			} else {
				if (SetWindowDisplayAffinity(target, WDA_NONE)) {
					gExcludedFromCapture.erase(it);
					sprintf(status, "已取消窗口[%s]的截图透明。", title);
					SetWindowText(TxOut, status);
					Println(status);
				} else {
					PrtError("取消截图透明失败。", 0);
					SetWindowText(TxOut, "取消截图透明失败。");
				}
			}
			break;
		}
