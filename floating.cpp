...
static void DrawFloatingContent(HDC hdc) {
    ...
    if (g_gdiOk && g_pBitmap) {
        ...
        DeleteObject(hBg);
        if (g_hFallbackIcon)
            DrawIconEx(hdc, 4, 4, g_hFallbackIcon, FLOAT_SIZE - 8, FLOAT_SIZE - 8, 0, NULL, DI_NORMAL);
    }
}
...
LRESULT CALLBACK FloatingWndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    switch (msg) {
        case WM_CREATE: {
            ...
            g_hFallbackIcon = LoadIcon(cs->hInstance, "MAINICON");
            InitGDIPlus();
